...
def pids(arg1):
    print(arg1)
    Pid1 = erlang.call(Atom(b'erlang'), Atom(b'self'), [])
    Pid2 = erlang.call(Atom(b'erlang'), Atom(b'self'), [])
    return [Pid1,Pid2]
...
class Agent(nn.Module):
    def __init__(self, obs_shape, action_space):
        ...
        self.network = nn.Sequential(
            layer_init(nn.Linear(np.array(obs_shape).prod(), 2048)),
            nn.Tanh(),
    ...
    def get_action_and_value(self, the_device, x, action_mask, lstm_state, done, action=None):
...
def go(obs_shape, action_shape, action_space, first_obs, next_mask, module_step):
    ...
    agent = Agent(obs_shape, action_space).to(device)
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                action, logprob, _, value, next_lstm_state = agent.get_action_and_value(
                    device, next_obs, action_masks[step], next_lstm_state, next_done)
